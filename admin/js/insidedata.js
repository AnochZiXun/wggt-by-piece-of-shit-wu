...
//多選刪除
	$('#delIcon').click(function(){
		if(confirm("是否刪除勾選資料"))
		{
			...
			$.post('./ajax/ajaxdel.php',{'table':$('.deleteMenu').attr('thistable') , 'item':delthisitem , 'postkind':'muchdata' , 'page':sendPage , 'path':$('#doc_path').val() , 's_column':$('#s_column').val() , 's_keyword':$('#s_keyword').val() , 's_keyword1':$('#s_keyword1').val() ,  'control':$('.deleteMenu').attr('thiscontrol')},function(response){
				//$('#mainContent').load('inside.php?control='+response.control+'&kind='+response.kind+'&p='+response.page+'&catelist='+$('#doc_path').val()+'&s_column='+$('#s_column').val()+'&s_keyword='+$('#s_keyword').val()+'&s_keyword1='+$('#s_keyword1').val()   ) ;
				$('#mainContent').load('inside.php?control='+response.control+'&kind='+response.kind+'&p='+response.page+'&catelist='+$('#doc_path').val()+'&s_column='+$('#s_column').val()+'&s_keyword='+$('div#text_keyword').text()+'&s_keyword1='+$('div#text_keyword1').text() ) ;
				//$('#mainContent').load('inside.php?control='+response.control+'&kind='+response.kind+'&p='+response.page+'&catelist='+$('#doc_path').val() ) ;
			},'json');
		}
	});
//檢視
...
//直接修改輸入框的值
	$('.columnShow .fastinput').change(function(){
		$.post('./ajax/ajaxinside.php',{'table':$(this).attr('thistable') , 'item':$(this).attr('thisitem') , 'name':$(this).attr('name') , 'value':$(this).val() },function(){},'json');
	}) ;
...
//分類選單
	...
	$('select[name^="datacategory"]').live('change' ,function(){
		...
		if( $(this).attr('name') == categoryname && changeitem != '' )
		{
			$.post('./ajax/insidefindcategory.php',{'table':$('#categoryTable').attr('value') , 'item':changeitem , 'nowlevel':$(this).attr('nowlevel') , 'topic':$('#categoryTopic').attr('value') },function(response){
				...
				var url = 'inside.php?control='+ $('#control').attr('value') +'&kind='+$('#kind').attr('value')+'&catelist='+response.catelist ;
				url = url.replace(/^.*#/, '');
				$.history.load(url);
				return false;
			},'json');
		...
		else if( $(this).attr('name') == 'datacategory1' && changeitem == '' )
		{
			var url = 'inside.php?control='+ $('#control').attr('value') +'&kind='+$('#kind').attr('value') ;
			url = url.replace(/^.*#/, '');
			$.history.load(url);
