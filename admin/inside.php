...
$(document).ready(function(){
	$path = '<?=$add_another_path?>' ;
	$('#mainContent').load( $path.replace("..",".") ) ;
});
</script>
<?
		break ;
...
?>
		<div id="category">
			<ul>
				<li id="categoryList"><span><img src="images/h.gif" class="categoryImg" /><h1>所有分類目錄</h1>
<?
if($add[$_GET['kind']][0] != "false")
{
?>
				<img src="images/add_icon.png" width="16" height="16" class="categoryAdd" thishref="?control=<?=$control?>&cate_index=<?=$record['CATE_INDEX']?>&kind=0&page_status=add" />
<?
}
?>
				</span>
<?
			//遞迴樹狀結構
			function getCategory($table , $control , $cate_index , $kind)
			{
				global $db , $product_set_para_en , $sort , $add , $modefy , $delete;
				$rows = $db -> getcount("select * from ".$table." where DELETE_ID =0 and CATE_INDEX ='".$cate_index."'") ;
				if($rows != 0)
				{
					echo "<ul>" ;
				}
				$result = $db -> query("select * from ".$table." where DELETE_ID =0 and CATE_INDEX ='".$cate_index."' order by SORT ASC") ;
				while($record = $db -> getarray($result))
				{
					echo "<li><span>" ;
					if($add[$kind][$record['DOC_LEVEL']] != "false"){
						echo "<img src=\"images/h.gif\" class=\"categoryImg\" />" ;
					}else{
						echo "<p style=\"float:left;width:15px;\">&nbsp;</p>";
					}
					echo "<h2 thishref=\"?control=".$control."&item=".$record['ITEM']."&cate_index=".$record['CATE_INDEX']."&kind=0&page_status=show\">".$record[strtoupper($product_set_para_en[$kind][0])]."</h2>" ;

					//echo "<li><span><img src=\"images/h.gif\" class=\"categoryImg\" /><h2 thishref=\"?control=".$control."&item=".$record['ITEM']."&cate_index=".$record['CATE_INDEX']."&kind=0&page_status=show\">".$record[strtoupper($product_set_para_en[$kind][0])]."</h2>" ;
					if($add[$kind][$record['DOC_LEVEL']] != "false")
					{//新增
						echo "<img src=\"images/add_icon.png\" width=\"16\" height=\"16\" class=\"categoryAdd\" thishref=\"?control=".$control."&item=".$record['ITEM']."&kind=0&page_status=add\" />" ;
					}
					if($modefy[$kind][$record['DOC_LEVEL']] != "false")
					{//修改
						echo "<img src=\"images/edit.gif\" width=\"16\" height=\"16\" class=\"categoryModefy\" thishref=\"?control=".$control."&item=".$record['ITEM']."&kind=0&page_status=modefy\" />" ;
					}
					if($delete[$kind][$record['DOC_LEVEL']] != "false")
					{//刪除
						echo "<img src=\"images/del_icon.png\" width=\"16\" height=\"16\" class=\"categoryDelete\" thistable=\"".$table."\" thisitem=\"".$record['ITEM']."\" />" ;
					}
					if($sort[$kind][$record['DOC_LEVEL']] != "false")
					{//排序
						echo "<img src=\"images/up.gif\" class=\"sortup\" nowitem=\"".$record['ITEM']."\" thistable=\"".$table."\" thiskind=\"0\" thiscontrol=\"".$_GET['control']."\"  /><img src=\"images/down.gif\" class=\"sortdown\" nowitem=\"".$record['ITEM']."\" thistable=\"".$table."\" thiskind=\"0\" thiscontrol=\"".$_GET['control']."\" />" ;
					}
					echo "</span>" ;
					$nextcate = $record['ITEM'] ;
					getCategory($table , $control , $nextcate , $kind) ;
				}
				if($rows != 0)
				{
					echo "</ul>" ;
			...
			echo getCategory( $product_set_table[$_GET['kind']] , $_GET['control'] , $_GET['cate_index'] , $_GET['kind'] ) ;
?>
				</li>
			</ul>
		</div>
<script type="text/javascript" src="js/insidecategory.js"></script>
<?
		}
		else
		{
...
?>
			<div id="methodMenu">
<?
if( $excel[$_GET['kind']] == "true" )
{
?>
				<img src="images/excel.gif" style="cursor:pointer;" id="excelIcon" thisControl="<?=$_GET['control']?>" thisTable="<?=$product_set_table[1]?>" />
<?
}
...
{
?>
				<img src="images/sort.gif" style="cursor:pointer;" id="sortIcon" thishref="?control=<?=$_GET['control']?>&kind=<?=$_GET['kind']?>&catelist=<?=$_GET['catelist']?>" />
<?
}
if( $delete[$_GET['kind']] != "false" )
{
...
<?
	for($sum = 0 ; $sum < $columnTotal ; $sum++)
	{
		if( $sum == 0 )
		{
			$sumFirst = explode("%" , $columnWeight[0] ) ;
			//echo "<p style=\"width:".($sumFirst[0] - 1)."%;\">" ;
			echo "<font style=\"float:left;width:".($sumFirst[0] - 1)."%;\">".$product_set_para_ch[$_GET['kind']][$columnPName[$sum]]."</font>" ;
		}
		else
		{
...
<?
				...
				{
?>
					<img src="images/modefy.gif" class="modefyMenu" thishref="?control=<?=$_GET['control']?>&item=<?=$record['ITEM']?>&kind=<?=$_GET['kind']?>&page_status=modefy&p=<?=$_GET['p']?>&catelist=<?=$_GET['catelist']?>&s_column=<?=$_GET['s_column']?>&s_keyword=<?=$_GET['s_keyword']?>&s_keyword1=<?=$_GET['s_keyword1']?>" style="cursor:pointer;" />
<?
				}
?>
				</span>
...
?>
				<span class="sort"><img src="images/up.gif" class="<?=$sortup?>" nowitem="<?=$record['ITEM']?>" thistable="<?=$product_set_table[$_GET['kind']]?>" nowpage="<?=$_GET['p']?>" thiskind="1" thiscontrol="<?=$_GET['control']?>" style="cursor:pointer;" /></span>
				<span class="sort"><img src="images/down.gif" class="<?=$sortdown?>" nowitem="<?=$record['ITEM']?>" thistable="<?=$product_set_table[$_GET['kind']]?>" nowpage="<?=$_GET['p']?>" thiskind="1" thiscontrol="<?=$_GET['control']?>" style="cursor:pointer;" /></span>
<?
				}
?>
				...
				if($changePrintColumn == "true")
				{
					for($sum = 0 ; $sum < $columnTotal ; $sum++)
					{
						...
						if($product_set_para_ex[$_GET['kind']][$columnPName[$sum]] == 1 || $product_set_para_ex[$_GET['kind']][$columnPName[$sum]] == 4)
						{
							...
							{
								if( $ModefyInputColumn == "true" )
								{
									if( $columnPName[$sum] == $inputModefy[$sum] )
									{
										echo "<input class=\"fastinput\" style=\"width:80%;\" type=\"text\" name=\"".$product_set_para_en[$_GET['kind']][$columnPName[$sum]]."\" thistable=\"".$product_set_table[$_GET['kind']]."\" thisitem=\"".$record['ITEM']."\" value=\"".$record[strtoupper($product_set_para_en[$_GET['kind']][$columnPName[$sum]])]."\">" ;
									}
									else
									{
...
?>
<input type="hidden" id="doc_path" name="doc_path" value="<?=$_GET['catelist']?>" />
<input type="hidden" id="s_column" name="s_column" value="<?=$_GET['s_column']?>" />



<div id="text_keyword" style="display:none;"><?=$s_keyword?></div>
<div id="text_keyword1" style="display:none;"><?=$s_keyword1?></div>



<link href="js/lightbox.css" rel="stylesheet" type="text/css" />
<script type="text/javascript" src="js/jquery.lightbox.js"></script>
<script type="text/javascript" src="js/insidedata.js"></script>
<?
		break ;
}
?>
